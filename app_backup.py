# app_streamlit.py
# Streamlit UI untuk BeautyProductRecommendationSystem (versi final dengan semua data & gambar)
...
class BeautyProductRecommendationSystem:
    ...
    def _normalize_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ["nama_produk", "brand", "sub_kategori", "manfaat", "jenis_kulit_kompatibel", "deskripsi", "klaim", "url_gambar"]:
            if col in df.columns:
                df[col] = df[col].astype(str).fillna('')
        return df

    def load_and_preprocess_data(self):
        # ---- SEMUA DATA ANDA YANG SUDAH LENGKAP DENGAN GAMBAR ----
        data = [
            # Skintific
            [1, "Skintific", "Skintific", "Panthenol Cleanser", "Sabun cuci muka", "Kulit jerawat & Kemerahan", 5.00, "Panthenol: menenangkan iritasi kulit", 89000, 120, "Hydrating|Brightening", "https://www.soco.id/cdn-cgi/image/w=360,format=auto,quality=100,dpr=1.5/https://images.soco.id/993ce8cb-4150-4b49-a9b2-2c9b77d55a7c-.jpg"],
            ...
            [1, "Skintific", "Skintific", "Daily Sunscreen", "Menenangkan dan melembabkan", "Kulit berjerawat", 5.00, "The Water-Like Elegant sunscreen yang ringan", 109000, 30, "Hydrating|Soothing", "https://images.tokopedia.net/img/cache/700/hDjmkQ/2023/10/26/1d505370-96f7-4148-8167-bd1c9258ac7f.jpg"],
            # Glad2Glow
        ...
        columns = ['id', 'nama_produk', 'brand', 'sub_kategori', 'Manfaat', 'jenis_kulit_kompatibel',
                   'rating', 'deskripsi', 'harga_idr', 'size_ml', 'klaim', 'url_gambar']
...
<style>
  ...
  .product-card .skin-type { font-size: 0.8rem; color: #7d2a4f; background-color: #fff0f7; padding: 0.2rem 0.5rem; border-radius: 5px; margin-bottom: 0.7rem; display: inline-block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }
  .product-card .manfaat, .product-card .deskripsi { font-size: 0.85rem; color: #666; margin-bottom: 0.5rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; min-height: 2.5em; }
...
try:
    # Langsung muat data dari dalam kode, tidak perlu upload file
    recomm.load_and_preprocess_data()
    st.sidebar.success(f"Data internal dimuat: {len(recomm.products_df)} produk.")
    ...
    if 'skin_tokens' in df and not df['skin_tokens'].empty:
        all_tokens = np.concatenate(df['skin_tokens'].values)
        skin_opts = sorted(pd.Series(all_tokens).unique())
                ...
                for i, (idx, score, prod) in enumerate(ranked):
                    with cols[i % 4]:
                        ...
                        <div class="product-card">
                            ...
                            <h3>{prod.get('sub_kategori', 'N/A')}</h3>
                            <div class="price">Rp {prod.get('harga_idr', 0):,.0f}</div>
                            <div class="details">⭐ {prod.get('rating', 0):.1f} | {prod.get('size_ml', 0):.0f} ml</div>
                            <div class="skin-type" title="{jenis_kulit}">Cocok untuk: {jenis_kulit}</div>
                            <div class="manfaat"><b>Manfaat:</b> {prod.get('Manfaat', '')}</div>
                            <div class="deskripsi"><b>Deskripsi:</b> {prod.get('deskripsi', '')}</div>
